...
async function searchStudents(){
  ...
  querySnapshot.forEach(docSnap=>{
    ...
    st.problems.forEach(p=>{
      let match=false;
      if(q === p.teacher.toLowerCase()){
        match = true;
      } else if(q === p.part.toLowerCase()){
        match = true;
      } else if(q === p.subject.toLowerCase()){
        match = true;
      } else if(q === st.name.toLowerCase()){
        match = true;
      }
      if(match){
        h+=`<tr>
  ...
  if(h==='<tr><th>الاسم</th><th>الفصل</th><th>السكن</th><th>رقم الهاتف</th><th>المادة</th><th>الجزئية</th><th>الأستاذ</th></tr>'){
...
async function createSession(){
  ...
  querySnapshot.forEach(docSnap=>{
    const st={id:docSnap.id,...docSnap.data()};
    st.problems.forEach(p=>{
      const matchTeacher = teacher ? p.teacher.toLowerCase()===teacher : true;
      const matchSubject = subject ? p.subject.toLowerCase()===subject : true;
      const matchPart = part ? p.part.toLowerCase()===part : true;
